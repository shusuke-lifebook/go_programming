...
### 8-5-3 ミドルウェアを追加しよう
- HTTPリクエストを受け取ってから、HTTPリクエストを返すまでの間に挟み込まれる処理のことを**ミドルウェア**という
- ginパッケージで使われるミドルウェアを追加しアプリケーションに機能を追加していく

- **CORSを設定するミドルウェア**
  - github.com/gin-contrib/corsは、**CORS(Cross-Origin Resource Sharing)**という異なるオリジンからのリクエストを許可する仕組みサポートするパッケージ
- **タイムアウトを設定するミドルウェア**
  - github.com/gin-contrib/timeoutは、ハンドラーの処理が指定した時間内に終了したなかった場合、処理を中断(**タイムアウト**)するためのパッケージです。
- **ロギングのためのミドルウェア**
  - github.com/gin-contrib/zapは、高速で構造化されたロギングライブラリを使ってリクエストやレスポンス、パニックなどの情報をログに出力するパッケージ。
  - **ginzap.Ginzap関数**は、リクエストやレスポンスの情報をログに出力するための関数。
  - **ginzap.RecoveryWithZap関数**は、パニックが発生した場合に、ログにエラーとスタックトレースを記録するための関数。
  ...
  func timeoutMiddleware(duration time.Duration) gin.HandlerFunc {
    return timeout.New(
      ...
      timeout.WithResponse(func(c *gin.Context) {
        c.JSON(http.StatusRequestTimeout, api.ErrorResponse{Message: "timeout"})
        c.Abort()
      }),
    )
  }
